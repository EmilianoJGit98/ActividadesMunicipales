...
export class ConfirmarInscripcionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      this.ArrActividades = params['actividadesSeleccionadas'] ? JSON.parse(params['actividadesSeleccionadas']) : null;
  ...
  private async renderComponent(bricksBuilder: any, preferenceId: string): Promise<void> {
    ...
    await bricksBuilder.create('wallet', 'formActividadbtn', {
      ...
      callbacks: {
        onError: (error: any) => console.error('Error en el botón de pago:', error),
        onReady: () => {
  ...
  private async checkPayment(paymentId: number, participanteId: number) {
    ...
    if (!isNaN(this.paymentId)) {
      ...
      this.pagoPersonalizado.getPayment(this.paymentId, this.participanteId).subscribe({  
        next: (res: any) => {  
